#!/bin/bash
# Complete Pwnagotchi Installation Script for Pi Zero 2W + Waveshare v4
# Based on custom development session - includes all patches and fixes
# Version: 3.0 - AI Enhanced with Real Attack Support

set -e

echo "🤖 Complete Pwnagotchi Installer v3.0"
echo "======================================"
echo "For: Raspberry Pi Zero 2W + Waveshare v4 Display"
echo "Features: AI Learning, Real Attacks, Web Interface"
echo ""
...
if [[ $EUID -eq 0 ]]; then
   error "This script should not be run as root. Run as pi user."
   exit 1
fi

# Check if Pi Zero 2W
if ! grep -q "Pi Zero 2" /proc/device-tree/model 2>/dev/null; then
    warn "This script is optimized for Pi Zero 2W. Continue anyway? (y/n)"
    read -r response
    if [[ ! "$response" =~ ^[Yy]$ ]]; then
        exit 1
    fi
...
log "Starting Complete Pwnagotchi installation..."
...
log "Creating Pwnagotchi directories..."
sudo mkdir -p /etc/pwnagotchi
sudo mkdir -p /var/log
sudo mkdir -p /tmp/handshakes
sudo chmod 755 /tmp/handshakes

# Create the main Pwnagotchi application
log "Creating Complete Pwnagotchi application..."
sudo tee /usr/local/bin/complete_pwnagotchi.py > /dev/null << 'PYTHON_EOF'
#!/usr/bin/env python3
"""
...
sudo chmod +x /usr/local/bin/complete_pwnagotchi.py
#!/bin/bash
# Complete Pwnagotchi Installation Script - Part 2
# This completes the installation after dependencies are installed

# Continue from pwnagotchi_installer.sh

# Create the main Pwnagotchi application
log "Creating Complete Pwnagotchi application..."
sudo tee /usr/local/bin/complete_pwnagotchi.py > /dev/null << 'PYTHON_EOF'
# The complete_pwnagotchi.py content goes here
# (This would be the content from /tmp/complete_pwnagotchi.py)
PYTHON_EOF
...
sudo tee /etc/systemd/system/pwnagotchi.service > /dev/null << 'SERVICE_EOF'
[Unit]
Description=Complete Pwnagotchi v3.0 - AI WiFi Auditing Tool
After=network.target

[Service]
Type=simple
User=root
ExecStart=/usr/bin/python3 /usr/local/bin/complete_pwnagotchi.py
Restart=always
RestartSec=10
...
log "Enabling Pwnagotchi service..."
...
log "Enabling SPI for Waveshare display..."
...
sudo tee /usr/local/bin/pwnagotchi-start > /dev/null << 'START_EOF'
#!/bin/bash
echo "🤖 Starting Complete Pwnagotchi..."
sudo systemctl start pwnagotchi.service
sudo systemctl status pwnagotchi.service --no-pager
echo ""
echo "🌐 Web Interface: http://$(hostname -I | awk '{print $1}'):8080"
echo "📺 Display: Waveshare v4 should show status"
echo "📝 Logs: sudo journalctl -u pwnagotchi.service -f"
START_EOF

sudo tee /usr/local/bin/pwnagotchi-stop > /dev/null << 'STOP_EOF'
#!/bin/bash
echo "🛑 Stopping Complete Pwnagotchi..."
sudo systemctl stop pwnagotchi.service
sudo systemctl status pwnagotchi.service --no-pager
STOP_EOF

sudo tee /usr/local/bin/pwnagotchi-status > /dev/null << 'STATUS_EOF'
#!/bin/bash
echo "📊 Complete Pwnagotchi Status:"
echo "================================"
sudo systemctl status pwnagotchi.service --no-pager
echo ""
echo "🌐 Web Interface: http://$(hostname -I | awk '{print $1}'):8080"
echo "📝 Recent Logs:"
sudo journalctl -u pwnagotchi.service --no-pager -n 10
STATUS_EOF

sudo tee /usr/local/bin/pwnagotchi-logs > /dev/null << 'LOGS_EOF'
#!/bin/bash
echo "📝 Complete Pwnagotchi Live Logs:"
echo "================================="
echo "Press Ctrl+C to exit"
sudo journalctl -u pwnagotchi.service -f
LOGS_EOF

sudo chmod +x /usr/local/bin/pwnagotchi-*

# Create monitor mode setup script for when external adapter arrives
log "Creating monitor mode setup script..."
sudo tee /usr/local/bin/setup-monitor-mode > /dev/null << 'MONITOR_EOF'
#!/bin/bash
echo "🔥 Setting up Monitor Mode for Real Attacks"
echo "==========================================="

# Check for external WiFi adapter
if iwconfig 2>/dev/null | grep -q wlan1; then
    echo "✅ External WiFi adapter detected (wlan1)"
    ...
    if iwconfig 2>/dev/null | grep -q wlan1mon; then
        echo "✅ Monitor mode enabled successfully!"
        echo "🔥 REAL ATTACK MODE will be activated on next start"
    else
    ...
    # Restart pwnagotchi
    echo "🚀 Restarting Pwnagotchi with real attack mode..."
    sudo systemctl start pwnagotchi.service
    
    echo ""
    echo "🎯 Real Attack Mode Active!"
    echo "🌐 Web Interface: http://$(hostname -I | awk '{print $1}'):8080"
    
else
    echo "❌ No external WiFi adapter found"
    echo "💡 Connect Alfa AWUS036ACS or compatible adapter"
    echo "📋 Current interfaces:"
    iwconfig 2>/dev/null | grep -E "^[a-z]"
fi
MONITOR_EOF

sudo chmod +x /usr/local/bin/setup-monitor-mode

# Create quick test script
log "Creating test script..."
sudo tee /usr/local/bin/test-pwnagotchi > /dev/null << 'TEST_EOF'
#!/bin/bash
echo "🧪 Testing Complete Pwnagotchi Installation"
echo "=========================================="

echo "1. Checking Python dependencies..."
python3 -c "
try:
    import flask, scapy, PIL
    from waveshare_epd import epd2in13_V4
    import RPi.GPIO
...
which airmon-ng >/dev/null && echo "✅ airmon-ng available" || echo "❌ airmon-ng missing"
which aireplay-ng >/dev/null && echo "✅ aireplay-ng available" || echo "❌ aireplay-ng missing"
which iwlist >/dev/null && echo "✅ iwlist available" || echo "❌ iwlist missing"

echo ""
echo "3. Checking WiFi interfaces..."
...
sudo systemctl is-enabled pwnagotchi.service >/dev/null && echo "✅ Service enabled" || echo "❌ Service not enabled"

echo ""
echo "5. Testing web server (10 second test)..."
...
" &
sleep 2
if curl -s http://localhost:8081/ | grep -q "Test OK"; then
...
echo "🚀 Start with: pwnagotchi-start"
TEST_EOF

sudo chmod +x /usr/local/bin/test-pwnagotchi

# Final setup
log "Final setup and permissions..."
sudo chown -R root:root /usr/local/bin/complete_pwnagotchi.py
sudo chown -R root:root /usr/local/bin/pwnagotchi-*
sudo chown -R root:root /usr/local/bin/setup-monitor-mode
sudo chown -R root:root /usr/local/bin/test-pwnagotchi

echo ""
echo "🎉 Complete Pwnagotchi Installation Finished!"
echo "============================================="
echo ""
echo "📋 Quick Start Commands:"
echo "  pwnagotchi-start    - Start Pwnagotchi"
echo "  pwnagotchi-stop     - Stop Pwnagotchi"  
echo "  pwnagotchi-status   - Check status"
echo "  pwnagotchi-logs     - View live logs"
echo "  test-pwnagotchi     - Test installation"
echo "  setup-monitor-mode  - Enable real attacks (when external WiFi adapter connected)"
echo ""
echo "🌐 Web Interface: http://$(hostname -I | awk '{print $1}'):8080"
echo "📺 Display: Waveshare v4 will show status"
echo ""
echo "🔥 Features:"
echo "  ✅ AI Learning and Target Selection"
echo "  ✅ Waveshare v4 Display Support"
echo "  ✅ Web Interface with Attack Controls"
echo "  ✅ Simulation Mode (current)"
echo "  ✅ Real Attack Mode (when external WiFi adapter connected)"
echo ""
echo "🚀 Start now: pwnagotchi-start"
echo ""

# Auto-start if requested
read -p "🤖 Start Pwnagotchi now? (y/n): " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    pwnagotchi-start
fi
